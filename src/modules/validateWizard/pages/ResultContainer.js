        ...
        {normalizedReport.signatures.length !== 0 ? <div className="container text-center">
            <div className="row validateResult py-0 mt-0">
                ...
                <div className="col-2 px-0 text-nowrap"><b><FormattedMessage id="validation.sign.format" defaultMessage="EU Format" /></b>
                    <img title={intl.formatMessage({ id: "more.info", defaultMessage: "More information" })} style={{ width: 16, height: 16, marginBottom: 3, marginLeft: 4 }} src="/img/questionMark.svg" onClick={() => setShowPkcs7Info(!showPkcs7Info)} />
                    {showPkcs7Info && <div className="text-wrap" style={{ textAlign: "left", marginLeft: "-170px", backgroundColor: "white", width: "400px", borderStyle: "dotted", padding: "10px", position: "absolute", zIndex: 1 }}>
                        <FormattedMessage id="validation.sign.pkcs7.info" defaultMessage="This signature validation is based on the {LINK} and complies with the {LINK1}.{br}Other signature formats are available on the market (for example PKCS7 in Adobe Reader).{br}Please check the validation standard requested for your validation."
                            values={{
                                br: <br />, LINK: (<a href="https://ec.europa.eu/digital-building-blocks/DSS/webapp-demo">
                                ...
                                LINK1: (<a href="https://www.etsi.org/deliver/etsi_EN/319100_319199/31910201/01.03.01_60/en_31910201v010301p.pdf">
                                    <FormattedMessage id="validation.sign.pkcs7.linkText1" defaultMessage="applicable European standards for signature validation" /></a>)
                            }}
                        />
            ...
            {normalizedReport.signatures.map((sig, index) => <div key={index} className={"row validateResult " + ("light-" + (sig.missingSigningCert ? "pkcs7" : (sig.valid ? (sig.qualified ? "success" : "warning") : "danger")))}>
                ...
                <div className="col-2 px-0"> {
                    sig.missingSigningCert ? <strong>? *</strong> : <>
                        {sig.valid ? yes : no}
                        {sig.subIndication && <img title={intl.formatMessage(
                            subIndication[sig.subIndication] ?
                                { id: subIndication[sig.subIndication].id, defaultMessage: subIndication[sig.subIndication].message }
                                : { id: "error.title.short", defaultMessage: "Error" }
