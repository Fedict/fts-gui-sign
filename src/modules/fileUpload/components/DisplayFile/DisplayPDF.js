...
import { drawPdfSignature, RED, GREEN, BLACK, WHITE } from "./V3PdfSignature";
...
export const DisplayPDF = ({ file, drawSignature }) => {
    ...
    const DateOfSignature = { "de": "Datum der Unterschrift", "en": "Date of signature", "fr": "Date de signature", "nl": "Datum van handtekening" };
    const SignatureDate = { "de": "D MMM YYYYY [um] HH:mm [UTC]", "en": "[On] D MMM YYYY [at] HH:mm [UTC]", "fr": "[Le] D MMM YYYYY [à] HH:mm [UTC]", "nl": "[Op] D MMM YYYY [aan] HH:mm [UTC]" };

    const drawSignatureRect = (ctx, r, isSelected, isValid = true) => {
        const texts = [DateOfSignature[language] || DateOfSignature.en, moment().utc().format(SignatureDate[language] || SignatureDate.en), "Nora Angèle", "Specimen"];
        drawPdfSignature({ x: r.left, y: r.top, w: r.right - r.left, h: r.bottom - r.top }, isValid ? (isSelected ? GREEN : WHITE) : RED, texts, ctx)
    }
